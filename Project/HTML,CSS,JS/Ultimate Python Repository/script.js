const REPO_OWNER = 'abhisek2004';
const REPO_NAME = '62Days-CodeSprint-WebDev-Challenge';
    ...
    for (const item of immediateContents) {
        ...
        if (item.type === "blob") { 
            ...
            if (internalChildren.length === 1 && internalChildren[0].type === "blob") {
                ...
                // FIXED: Clicking anywhere on the entire container option now navigates and expands the dropdown menu list
...
fn renderImageBlock(fileName, downloadUrl, containerElement, elementId) {
    const block = document.createElement('div');
